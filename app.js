...
// ---- Solver hookup ----
document.getElementById('solveBtn').addEventListener('click', ()=>{ for(const f of facesOrder){ if(!capturedFaces[f] || capturedFaces[f].some(x=>!x)){ alert('Skanna och fyll alla 6 sidor först.'); return; } } solverWorker.postMessage({ type:'solve', state:capturedFaces }); noteEl.textContent='Beräknar lösning (demo)...'; });
