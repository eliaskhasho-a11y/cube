...
tabIds.forEach(id=> document.getElementById("tab-"+id).addEventListener("click", ()=> goto(id)));
...
try { solverWorker = new Worker("/solver_worker.js"); } catch (e) { console.warn("Solver worker disabled:", e); }
...
if (solverWorker) {
  solverWorker.onmessage = (e)=>{
    ...
    if(type==='solution'){ renderSolution(moves); noteEl.textContent = note || ''; }
  };
}
...
async function startCamera(){
  try{
    ...
    statusEl.textContent = "Kunde inte starta kameran. Tillåt kameran i Safari.";
  }
}
...
captureBtn.addEventListener('click', ()=>{ const colors=sampleFaceFromVideo(); if(!colors){ statusEl.textContent='Ingen bild – klicka Starta kamera först'; return; } const mapped=colors.map(nearestColor); const face=faceSelect.value; capturedFaces[face]=mapped; saveFaces(); });
calibBtn.addEventListener('click', ()=>{ const imgd=sampleSquare(); const gain=grayWorldGain(imgd); calibration={gain}; saveCalibration(); statusEl.textContent='Kalibrerad vitbalans ✓'; setTimeout(()=> statusEl.textContent='Kamera igång – rikta rutnätet mot en sida', 1500); });
...
function toggleTimer(){ if(!timerRunning){ timerRunning=true; startTime=performance.now(); startStopBtn.textContent='Stop'; updateTimer(); } else { timerRunning=false; const t=(performance.now()-startTime)/1000; times.push(parseFloat(t.toFixed(2))); saveTimes(); startStopBtn.textContent='Start'; scrambleEl.textContent=newScramble(); } }
...
function renderStats(){ const pb = times.length? Math.min(...times): null; const a5=avgN(5), a12=avgN(12); statsEl.innerHTML = `<div class="statline"><div>PB: <strong>${formatTime(pb)}</strong></div><div>Ao5: <strong>${formatTime(a5)}</strong></div><div>Ao12: <strong>${formatTime(a12)}</strong></div><div>Antal: <strong>${times.length}</strong></div></div>`; }
startStopBtn.addEventListener('click', toggleTimer);
resetBtn.addEventListener('click', resetTimer);
clearTimesBtn.addEventListener('click', clearTimes);

// ---- Data ----
exportBtn.addEventListener('click', ()=>{ const data={faces:capturedFaces,times,calibration}; const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='gcube-data-v3_1.json'; a.click(); URL.revokeObjectURL(url); });
...
// ---- Coach ----
function renderCoach(){ coachList.innerHTML=''; const a12=avgN(12), a5=avgN(5); const suggestions=[]; if(times.length>=12 && a5 && a12 && a5>a12*1.03) suggestions.push('Fokusera på jämnhet: 5 långsamma solves utan lockups.'); suggestions.push('OLL: Sune & Antisune 3×5'); suggestions.push('PLL: T-perm & U-perm 3×5'); suggestions.forEach(t=>{ const li=document.createElement('li'); li.textContent=t; coachList.appendChild(li); }); }
...
function drawCube(){ const w=cubeCanvas.width=340, h=cubeCanvas.height=220; const c=cubeCtx; c.clearRect(0,0,w,h); const U=capturedFaces.U||Array(9).fill('W'); const F=capturedFaces.F||Array(9).fill('G'); const R=capturedFaces.R||Array(9).fill('R'); drawFace(c, 30,10,90,U); drawFace(c, 30,110,90,F); drawFace(c, 130,110,90,R); }

// ---- Solver hookup ----
document.getElementById('solveBtn').addEventListener('click', ()=>{ for(const f of facesOrder){ if(!capturedFaces[f] || capturedFaces[f].some(x=>!x)){ alert('Skanna och fyll alla 6 sidor först.'); return; } } if (solverWorker) { solverWorker.postMessage({ type:'solve', state:capturedFaces }); noteEl.textContent='Beräknar lösning (demo)...'; } else { noteEl.textContent='Solver ej aktiv (iOS begränsning eller blockerad).'; } });
function renderSolution(moves){ solutionList.innerHTML=''; moves.forEach((m,i)=>{ const li=document.createElement('li'); li.textContent=`${i+1}. ${m}`; solutionList.appendChild(li); }); }

// ---- Navigation ----
function goto(page){ setActive(page); if(page==='scan'){ startOverlay.style.display='flex'; statusEl.textContent='Tryck “Starta kamera” för att ge åtkomst.'; } else { stopCamera(); } if(page==='cube') drawCube(); }

// ---- Init ----
if('serviceWorker' in navigator){ window.addEventListener('load', ()=> navigator.serviceWorker.register('/sw.js').catch(()=>{})); }
window.addEventListener('load', ()=>{ renderFacesList(); renderStats(); renderCoach(); scrambleEl.textContent=newScramble(); goto('scan'); });